...
// * test tools for developing.
function convertPrint(doc) {
  let map = doc.geometry.coordinates
  let name = doc.properties.MARKNAME1
  printjson(`${name} :  ${map}`)

  let convertMap = twd97_to_wgs84(map[0], map[1])
  printjson(`${name} :  ${convertMap}`)
}

// * Main function
// update a document.  TWD97 --> WGS84.
function updateFromTwd97ToWGS84(doc) {
  let map = doc.geometry.coordinates
  ...
  db[collection].update(
    { "_id": doc._id }, // value 要是 ObjectId("XxxxXXxxxxx")，不能只是傳入 id string.
    {
      $set: {
        ...
        // $set 只更新特定 filed 不影響其他的 ; dot notation 很好用
      }
    }
  );
  print(doc.properties.MARKNAME1 + ` is converted and updated.`)
}
